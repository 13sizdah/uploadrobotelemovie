...
ظرفیت و فضای رزرو هر backend نیز از پنل قابل تنظیم است. ربات مصرف فایل اصلی، replica و صف در انتظار را محاسبه می‌کند و روی فضای فاقد ظرفیت آپلود نمی‌کند. تعداد کارهای replication در صف در بالای پنل نمایش داده می‌شود و صف پس از restart ادامه خواهد یافت.
...
پنل دارای Audit Log، تغییر رمز پایدار، بکاپ دستی قابل دانلود و بکاپ خودکار روزانه است. هفت بکاپ خودکار و ده بکاپ دستی آخر در `data/backups/` باقی می‌مانند. هشدار عبور دیسک از ۹۰٪ و ناسالم‌شدن S3 به مدیر تلگرام ارسال می‌شود.

در صفحه «صف انتقال» می‌توان replication را بدون حذف کارها متوقف و بعداً ادامه داد. هنگام آپلود فعال نیز دکمه «لغو آپلود» در تلگرام نمایش داده می‌شود؛ لغو، فایل موقت را حذف می‌کند و انتقال Multipart فعال را متوقف می‌سازد.
