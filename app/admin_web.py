...
class AdminWeb:
    ...
    def page(self, body: str) -> web.Response:
        ...
        *{{box-sizing:border-box}}body{{margin:0;background:#07152b;color:#f7faff;font:16px/1.7 Tahoma;padding:24px}}main{{max-width:760px;margin:auto}}.card{{background:#0d203a;border:1px solid #29405e;border-radius:20px;padding:24px;margin:16px 0}}h1,h2{{margin-top:0}}label{{display:block;margin-top:13px;color:#bdcbe0}}input,select{{width:100%;min-height:46px;border:1px solid #405775;border-radius:10px;background:#071426;color:white;padding:10px;direction:ltr}}button{{min-height:46px;border:0;border-radius:10px;background:#43d6c5;color:#06211e;font-weight:bold;padding:10px 18px;margin-top:18px;cursor:pointer}}button:focus-visible,input:focus-visible{{outline:3px solid #8cf7ea;outline-offset:2px}}.row{{display:grid;grid-template-columns:1fr 1fr;gap:12px}}.muted{{color:#aebed5}}.ok{{color:#70e7b8}}.bad{{color:#ff9d9d}}@media(max-width:600px){{.row{{grid-template-columns:1fr}}}}
    ...
    async def index(self, request: web.Request) -> web.Response:
        ...
        cards = "".join(
            f'<section class="card"><h2>{html.escape(item.name)}</h2><p>Bucket: <b>{html.escape(item.bucket)}</b></p><p>Endpoint: {html.escape(item.endpoint_url)}</p><p>Region: {html.escape(item.region)} | Priority: {item.priority}</p><p>Latency: {item.latency_ms:.0f} ms | خطا: {item.failures}</p><p class="{"ok" if item.enabled and item.unhealthy_until <= time.monotonic() else "bad"}">{"سالم" if item.enabled and item.unhealthy_until <= time.monotonic() else "موقتاً خارج از سرویس"}</p></section>'
            for item in self.manager.backends.values()
        ) or '<p class="muted">هنوز فضای S3 ثبت نشده است.</p>'
