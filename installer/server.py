...
def env_quote(value: str) -> str:
    # Single-quoted Compose env values are literal, so `$` in password hashes
    # and credentials is not interpreted as variable substitution.
    ...
    try:
        ...
        if config["storage_backend"] == "s3":
            s3_config = [{
                ...
                "priority": 100,
                "enabled": True,
                "capacity_bytes": int(float(config["s3_capacity_gb"] or "0") * 1024 ** 3),
                "reserve_bytes": int(float(config["s3_reserve_gb"] or "0") * 1024 ** 3),
            }]
            env_lines.extend([
                "STORAGE_BACKEND=s3",
...
if(n===3) root.innerHTML=`<h2>مرحله ۳: دامنه و دسترسی</h2><label>دامنه متصل‌شده به IP سرور</label><input id="domain" placeholder="download.example.com"><div class="row"><div><label>مدت نگهداری فایل (ساعت)</label><input id="ttl" type="number" min="1" value="24"></div><div><label>شناسه عددی مدیر ربات</label><input id="admin_id" inputmode="numeric" placeholder="123456789"></div></div><div class="row"><div><label>ساخت رمز پنل وب</label><input id="web_password" type="password" minlength="12" autocomplete="new-password" placeholder="حداقل ۱۲ کاراکتر" required></div><div><label>تکرار رمز پنل وب</label><input id="web_password_confirm" type="password" minlength="12" autocomplete="new-password" placeholder="رمز را دوباره وارد کنید" required></div></div><p class="muted">رمز خام ذخیره نمی‌شود؛ فقط هش امن PBKDF2 در تنظیمات قرار می‌گیرد.</p><label>پذیرش فقط فایل فورواردشده از منابع پنل</label><select id="forward_only"><option value="true">فعال</option><option value="false">غیرفعال</option></select><label>شناسه کاربران مجاز (اختیاری)</label><input id="allowed_ids" placeholder="12345,67890"><label>فعال‌سازی خودکار HTTPS</label><select id="ssl"><option value="yes">بله، گواهی رایگان بگیر</option><option value="no">خیر، بعداً انجام می‌دهم</option></select><label>ایمیل برای گواهی SSL</label><input id="email" type="email" placeholder="admin@example.com"><div id="access_err"></div><div class="actions"><button class="btn" onclick="accessNext()">تنظیم فضای ذخیره‌سازی</button></div>`;
if(n===4) root.innerHTML=`<h2>مرحله ۴: فضای ذخیره‌سازی</h2><label>نوع ذخیره‌سازی</label><select id="storage_backend"><option value="s3">فضای S3 (پیشنهادی)</option><option value="local">دیسک محلی</option></select><div class="row"><div><label>نام فضا</label><input id="s3_name" value="primary-r2"></div><div><label>Region</label><input id="s3_region" value="auto"></div></div><label>S3 Endpoint</label><input id="s3_endpoint" placeholder="https://ACCOUNT_ID.r2.cloudflarestorage.com"><label>Bucket</label><input id="s3_bucket"><div class="row"><div><label>Access Key</label><input id="s3_access_key"></div><div><label>Secret Key</label><input id="s3_secret_key" type="password"></div></div><div class="row"><div><label>سهمیه قابل استفاده (GB، صفر=نامحدود)</label><input id="s3_capacity_gb" type="number" min="0" step="0.1" value="0"></div><div><label>فضای رزرو (GB)</label><input id="s3_reserve_gb" type="number" min="0" step="0.1" value="0"></div></div><p class="muted">کلیدها فقط در فایل .env با دسترسی 600 ذخیره می‌شوند.</p><div class="actions"><button class="btn" onclick="wizard(5)">بررسی نهایی</button></div>`;
...
function accessNext(){{let p=values.web_password||'';let c=values.web_password_confirm||'';let box=document.getElementById('access_err');if(p.length<12){{box.innerHTML='<div class="error">رمز پنل باید حداقل ۱۲ کاراکتر باشد.</div>';return}}if(p!==c){{box.innerHTML='<div class="error">رمز و تکرار آن یکسان نیستند.</div>';return}}box.innerHTML='';wizard(4)}}
async function startInstall(){{values.ssl=values.ssl||'yes';values.ttl=values.ttl||'24';values.forward_only=values.forward_only||'true';values.storage_backend=values.storage_backend||'s3';values.s3_name=values.s3_name||'primary-r2';values.s3_region=values.s3_region||'auto';values.s3_capacity_gb=values.s3_capacity_gb||'0';values.s3_reserve_gb=values.s3_reserve_gb||'0'; let r=await fetch('/install?key='+encodeURIComponent(key),{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(values)}});let j=await r.json();if(!r.ok){{document.getElementById('err').innerHTML='<div class="error">'+j.error+'</div>';return}};dots(5);root.innerHTML='<h2>در حال نصب</h2><pre id="log">شروع…</pre>';poll()}}
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if not secrets.compare_digest(
                str(config.get("web_password", "")),
                str(config.get("web_password_confirm", "")),
            ): errors.append("رمز پنل وب و تکرار آن یکسان نیستند")
            ...
            if config.get("storage_backend") == "s3":
                ...
                capacity = float(config.get("s3_capacity_gb", 0) or 0)
                reserve = float(config.get("s3_reserve_gb", 0) or 0)
                if capacity < 0 or reserve < 0 or (capacity and reserve >= capacity): errors.append("ظرفیت یا فضای رزرو S3 معتبر نیست")
            ...
            if errors:
                ...
                return
            config = {k: str(config.get(k, "")).strip() for k in ("bot_token", "api_id", "api_hash", "domain", "ttl", "allowed_ids", "admin_id", "web_password", "forward_only", "ssl", "email", "storage_backend", "s3_name", "s3_endpoint", "s3_bucket", "s3_region", "s3_access_key", "s3_secret_key", "s3_capacity_gb", "s3_reserve_gb")}
            with LOCK:
                if INSTALLING:
